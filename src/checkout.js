...
function checkout(config, substation, PARAMETERS, ORDER, VIRTUALMAINID, utility, tags, MESSAGESCODE, emitter) {
    ...
    let writeMesDataCompletedFlag = true;
    ...
    return {
        ...
        processStationData(data1) {
            ...
            if (previousvalidatebit === 0 && data[staticTags.CHECKOUTTRIGGER_TAG] === 1) {
                ...
                this.write = false;
                this.reset = false;
                log.error(`checkout startTime : ${Date.parse(new Date())}`);
                ...
                log.error(`checkout endTime : ${Date.parse(new Date())}`);
            }
        },
        /**
        ...
        */
        ...
        resetAck(data) {
            ...
            const isCheckOutCompleted = (0 === data[staticTags.CHECKOUTCOMPLETED_TAG])
          && (0 === data[staticTags.CHECKOUTRESULT_TAG])
          && (0 === data[staticTags.TARGETSUBSTATION_TAG]) ? true : false;
         ...
         */
         setTargetStation(plcdata, substationID) {
          if (substationID) {
      ...
      prepareProcessData() {
        ...
        if (CheckOutResult != null) {
          let payload = {};
          payload[parameterTags.SUBSTATIONID_TAG] = substationid;
          payload.name = staticTags.CHECKOUTRESULT_TAG;
          payload.value = CheckOutResult;
          obj.push(payload);
        }
        let payload = {};
        payload[parameterTags.SUBSTATIONID_TAG] = substationid;
        payload.name = staticTags.CHECKOUTCOMPLETED_TAG;
        payload.value = 1;
        if (CheckOutResult == 0) {
          payload.value = 0;
        }
            ...
            try {
                ...
                */
                ...
                if (response.status === utility.HTTP_STATUS_CODE.SUCCESS) {
                    ...
                    this.setTargetStation(plcdata, targetSubstation);
                    // write record
        ...
        */
        ...
        async writeProductionDataInMES(data, partstatusRecord, substationtype, componentParameterArray, runningOrder) {
            try {
                if (!writeMesDataCompletedFlag) return;
                ...
                const bomlist = [];
                for (let k = 0;k < bomkeys.length;k++) {
                    if (typeof data[bomkeys[k]] !== 'undefined') {
                        bomlist.push(data[bomkeys[k]]);
                    } else {
                        bomlist.push('');
                    }
                }
                ...
                writeMesDataCompletedFlag = false;
                var startTime = new Date().getTime();
                log.error(`${head["part_no_id"]}: startTime: ${startTime}`)
                ...
                log.error(`${head["part_no_id"]}: endTime: ${endTime}`);
                log.error(`${head["part_no_id"]}: timeout：${endTime - startTime}`);
                writeMesDataCompletedFlag = true;
                // // process writeUpdate or update partstatus
                // if (substationtype === 'initialsubstation') {
                //     this.writeUpdatePartStatus(data, partstatusRecord, runningOrder);
                // } else {
