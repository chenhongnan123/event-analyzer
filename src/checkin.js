...
function checkin(
  ...
  return {
    ...
    processStationData(data1) {
      ...
      if (
        previousvalidatebit === 0 &&
        data[staticTags.CHECKINTRIGGER_TAG] === 1
      ) {
    ...
    resetAck(data) {
      ...
      const isCheckInCompleted =
        0 === data[staticTags.CHECKINCOMPLETED_TAG] &&
        0 === data[staticTags.CHECKINRESULT_TAG] &&
        0 === data[staticTags.TARGETSUBSTATION_TAG]
          ? true
          : false;
      ...
      if (!this.reset && resetCheckInCompletedFlag) {
        ...
        if (
          resetCheckInCompletedFlag &&
          !isCheckInCompleted &&
          resetCheckInCompletedCount < feebackWriteCount
        ) {
    ...
    async getRecordForMainIDInInitStation(plcdata, runningOrder) {
      ...
      let roadmapType =
        runningOrder.orderdata[orderdetailsTags.ROADMAPTYPE_TAG];
      if (!roadmapType) {
        // bypass
        ...
        try {
          ...
          const ordername =
            runningOrder.orderdata[orderdetailsTags.ORDERNAME_TAG];
          ...
          query += `%26%26${[checkoutTags.MAINID_TAG]}=="${
            plcdata[staticTags.MAINID_TAG]
          }"`;
          ...
          if (response.status === utility.HTTP_STATUS_CODE.SUCCESS) {
              ...
              if (isOrderAchive) {
                ...
                if (isRoadMap) {
                  ...
                  if (isRecipe) {
                    ...
                    if (
                      plcdata[staticTags.PROCESSCODE_TAG] != undefined &&
                      runningOrder.runningRoadMap.length > 0 &&
                      runningOrder.runningRoadMap[0][
                        roadmapTags["PROCESSCODE_TAG"]
                      ] !== undefined
                    ) {
                      CheckInProcessCode =
                        runningOrder.runningRoadMap[0][
                    ...
                    plcdata[checkinTags["CHECKINNGCODE_TAG"]] =
                      CheckInNGCode || 0;
                    plcdata[checkinTags["PROCESSCODE_TAG"]] =
                      CheckInProcessCode || 0;
                    plcdata[checkinTags["CHECKINRESULT_TAG"]] =
                      CheckInResultValues.OK;
                    // write checkin result to checkin element
                    this.writeRecordInSWX(
                      plcdata,
                      runningOrder,
            ...
            log.error(
              `Error in getting data from elementName : ${elementName} ${JSON.stringify(
                response.data
              )}`
            );
        ...
        if (response.status === utility.HTTP_STATUS_CODE.SUCCESS) {
          if (
            ...
            const mainidOrder = ORDER.runningOrder.filter((item) => {
              ...
              return (
                response.data.results[0][orderdetailsTags.ORDERNAME_TAG] ===
                orderdata[orderdetailsTags.ORDERNAME_TAG]
              );
            });
            if (mainidOrder.length > 0) {
              ...
              log.error(
                `No RunningOrder found for mainid ${
                  plcdata[staticTags.MAINID_TAG]
                }`
              );
              ...
              plcdata[checkinTags["PROCESSCODE_TAG"]] = CheckInProcessCode || 0;
              plcdata[checkinTags["CHECKINRESULT_TAG"]] =
                CheckInResultValues.ORDERNOK;
              // write checkin result to checkin element
              this.writeRecordInSWX(plcdata, {});
            }
          } else {
            log.error(
              `No record found in partstatus for mainid ${
                plcdata[staticTags.MAINID_TAG]
              }`
            );
            CheckInResult = CheckInResultValues.ERROR;
    ...
    async checkOverAllResult(plcdata, runningOrder, lastStationData) {
      ...
      if (ordername != lastStationData[checkoutTags.ORDERNAME_TAG]) {
        log.error(
          `${MESSAGESCODE.XXX03003} for ${plcdata[staticTags.MAINID_TAG]}`
        );
      ...
      if (
        lastStationData[checkoutTags.MODESTATUS_TAG] &&
        lastStationData[checkoutTags.MODESTATUS_TAG] !=
          defaults.normalmodestatus
      ) {
      ...
      if (
        plcdata[staticTags.PROCESSCODE_TAG] != undefined &&
        runningRoadMap.length > 0 &&
        runningRoadMap[0][roadmapTags["PROCESSCODE_TAG"]] !== undefined
      ) {
      ...
      if (status === CheckInResultValues.NG) {
        ...
        plcdata[checkinTags["PROCESSCODE_TAG"]] = CheckInProcessCode || 0;
        plcdata[checkinTags["CHECKINRESULT_TAG"]] =
          CheckInResultValues.TESTFAILED;
        // write checkin result to checkin element
        this.writeRecordInSWX(plcdata, runningOrder);
        ...
        if (runningRoadMap.length === 0) {
          ...
          plcdata[checkinTags["CHECKINRESULT_TAG"]] =
            CheckInResultValues.BYPASS;
          // write checkin result to checkin element
          const targetSubstation = await this.getTargetSubStationForSubstation(
            runningRoadMap,
            lastStationData,
            runningOrder
          );
          ...
          for (var i = 0; i < runningRoadMap.length; i++) {
            ...
            if (
              lastStationData[checkoutTags.SUBSTATIONID_TAG] ===
              runningRoadMap[i][roadmapTags.PRESUBSTATIONID_TAG]
            ) {
              isProcessed = true;
            }
          }
          if (isProcessed) {
            ...
            if (isRoadMap) {
              ...
              if (isRecipe) {
                ...
                plcdata[checkinTags["CHECKINRESULT_TAG"]] =
                  CheckInResultValues.OK;
                const targetSubstation =
                  await this.getTargetSubStationForSubstation(
                    runningRoadMap,
                    lastStationData,
                    runningOrder
                  );
            ...
            if (presubstation == 0) {
              if (!lastStationData[checkoutTags.SUBSTATIONID_TAG]) {
                log.error(
                  `Laststation result does not contain substationid and presubstation is 0 OK ${CheckInResultValues.OK}`
                );
                CheckInResult = CheckInResultValues.OK;
                plcdata[checkinTags["CHECKINNGCODE_TAG"]] = CheckInNGCode || 0;
                plcdata[checkinTags["PROCESSCODE_TAG"]] =
    ...
    async getTargetSubStationForSubstation(
      runningRoadMap,
      lastStationData,
      runningOrder
    ) {
      ...
      const roadmapid =
        lastStationData[checkoutTags.ROADMAPID_TAG] ||
        runningOrder.orderdata[orderdetailsTags.ROADMAPID_TAG];
      if (
        lastSubstationid &&
        (CheckInResult === CheckInResultValues.OK ||
          CheckInResult === CheckInResultValues.BYPASS)
      ) {
        targetSubstation = await ORDER.getTargetSubStationForSubstation(
          roadmapid,
          lastSubstationid
        );
      } else {
        // find the substation which does not have presubstation for writing target substation
        let targetSubstationInfo = await ORDER.getTargetSubStationForSubstation(
          roadmapid
        );
    ...
    checkProductTypeName(plcdata) {
      ...
      if (
        ...
        runningOrder.orderdata[orderdetailsTags.PRODUCTTYPENAME_TAG] ===
          plcdata[staticTags.PRODUCTTYPENAME_TAG]
      ) {
    ...
    checkOrderActualQuantity(plcdata, runningOrder) {
      if (runningOrder) {
        if (
          runningOrder.ordercount >=
            runningOrder.orderdata[orderdetailsTags.TARGETCOUNT_TAG] &&
          substation[substationTags.ISMAINLINE_TAG]
        ) {
          ...
          plcdata[checkinTags["PROCESSCODE_TAG"]] = CheckInProcessCode || 0;
          plcdata[checkinTags["CHECKINRESULT_TAG"]] =
            CheckInResultValues.TARGETCOUNTREACHED;
          // write checkin result to checkin element
          this.writeRecordInSWX(plcdata, runningOrder);
          return false;
        }
    ...
    async checkRoadMap(plcdata, runningRoadMap, lastStationData, runningOrder) {
      // const runningOrder = ORDER.runningOrder;
      if (
        !substation[substationTags.INITIALSUBSTATION_TAG] &&
        runningRoadMap.length === 0
      ) {
    ...
    checkRecipe(plcdata, runningOrder) {
      ...
      if (
        (orderProduct &&
          ...
          !orderProduct[0][recipedetailsTags.RECIPENAME_TAG]) ||
        (orderProduct &&
          orderProduct.length > 0 &&
          orderProduct[0][recipedetailsTags.RECIPENAME_TAG] ===
            plcdata[staticTags.RECIPENAME_TAG])
      ) {
        log.error(`Recipe Matched with Order Recipe Name`);
        return true;
      } else {
        log.error(
          `${MESSAGESCODE.XXX03020} with ${plcdata[staticTags.RECIPENAME_TAG]}`
        );
    ...
    getTargetSubStationID(arrayOfTargetSubstationID) {
      ...
      if (arrayOfTargetSubstationID.length > 0) {
        substationID =
          arrayOfTargetSubstationID[0][roadmapTags.SUBSTATIONID_TAG];
      }
      return substationID;
    },
    /**
     ...
     */
    setTargetStation(substationID) {
      if (substationID) {
        log.info(`Target substation is ${substationID}`);
        this.targetSubStation = +substationID.split("-")[1];
      } else {
        log.error(
          `Can not set or write targetsubstation to PLC or in checkin CheckInResult: ${CheckInResult} and targetsubstation : ${substationID}`
        );
        this.targetSubStation = 0;
      }
    },
    ...
    prepareProcessData(data) {
      let obj = data ? data : [];
      if (this.virtualMainId != null && this.virtualMainId != '') {
        let payload = {};
      ...
      if (this.targetSubStation != null) {
        let payload = {};
        payload[parameterTags.SUBSTATIONID_TAG] = substationid;
        payload.name = staticTags.TARGETSUBSTATION_TAG;
        payload.value = this.targetSubStation;
        obj.push(payload);
      }
      if (CheckInResult != null) {
        let payload = {};
        payload[parameterTags.SUBSTATIONID_TAG] = substationid;
        payload.name = staticTags.CHECKINRESULT_TAG;
        payload.value = CheckInResult;
        obj.push(payload);
      }
      let payload = {};
      payload[parameterTags.SUBSTATIONID_TAG] = substationid;
      payload.name = staticTags.CHECKINCOMPLETED_TAG;
      payload.value = 1;
      if (CheckInResult == 0) {
        payload.value = 0;
      }
    ...
    async writeRecordInSWX(data, runningOrder, targetSubStation) {
      ...
      if (targetSubStation) {
        plcdata[checkinTags.TARGETSUBSTATION_TAG] =
          this.getTargetSubStationID(targetSubStation);
      }
      ...
      try {
        ...
        if (response.status === utility.HTTP_STATUS_CODE.SUCCESS) {
          log.error(
            `Checkin Record saved successfully in ShopWorx ${JSON.stringify(
              response.data
            )}`
          );
          if (
            mainidConfig.writelastprocessdatatoplc &&
            CheckInResult == CheckInResultValues.OK
          ) {
            const result = await new Promise((resolve) => {
            ...
            if (result.state) {
              // write processdata
              this.setTargetStation(plcdata[checkinTags.TARGETSUBSTATION_TAG]);
              processDataToPLC = this.prepareProcessData(result.data);
            ...
            mainidConfig.writevirtualmainidtoplc &&
            CheckInResult == CheckInResultValues.OK
          ) {
            this.setVirtualMainId(plcdata[staticTags.MAINID_TAG]);
            this.setTargetStation(plcdata[checkinTags.TARGETSUBSTATION_TAG]);
            processDataToPLC = this.prepareProcessData();
            processDataToPLCFlag = true;
            this.writeProcessData();
          ...
          log.error(
            `Error in writing Checkin result in ShopWorx ${JSON.stringify(
              response.data
            )}`
          );
    ...
    async generateProcessData(plcdata, resolve) {
      let recipe = [];
      let elementName = 'process_substation-263';
      const runningOrder = ORDER.runningOrder[0];
      if (substationid === "substation-250" && runningOrder.orderdata.productname !== '140704000077') {
        recipe = [
          {
            name: "90ComponentID",
      ...
      } else if (substationid === "substation-264" && runningOrder.orderdata.productname !== '140704000077') {
        recipe = [
          {
            name: "90checko1result",
            writename: "90checko2result",
          },
        ];
      } else {
        resolve({
      ...
      let query = `query=${[checkoutTags.MAINID_TAG]}=="${
        plcdata[staticTags.MAINID_TAG]
      }"`;
      query += `&sortquery=createdTimestamp==-1&pagenumber=1&pagesize=1`;
      const response = await elementService.getElementRecords(
      ...
      if (response.status === utility.HTTP_STATUS_CODE.SUCCESS) {
        if (
          ...
          log.error(
            `No record found in checkout for traceability ${
              plcdata[staticTags.MAINID_TAG]
            }`
          );
          CheckInResult = CheckInResultValues.ERROR;
        ...
        utility.checkSessionExpired(response.data);
        log.error(
          `Error in getting  processdata from elementName : traceability ${JSON.stringify(
            response.data
          )}`
        );
        CheckInResult = CheckInResultValues.ERROR;
